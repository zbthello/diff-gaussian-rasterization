	...
	bool debug)
{
    ...
    // 根据每个像素的损失梯度计算损失梯度与二维平均位置、二次曲线矩阵、不透明度和高斯RGB的关系。
    // 如果我们得到的是预先计算的颜色而不是SH，那么就使用它们。
    ...
    // 处理剩下的预处理。预先计算的是协方差，还是一个缩放系数/旋转系数对？
    // 如果已预先计算，则传递该值。如果没有，请使用我们自己计算的那个。
